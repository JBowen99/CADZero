...
- **AI chat is real and streaming.** A single `useChat` instance lives in
  `ChatProvider` (`app/lib/ai-chat.tsx`), wrapped around the whole workspace in
  `home.tsx`. `ChatPanel`, `Toolbar`, `StatusBar` all consume it via
  `useChatContext()` — that's why the provider must stay above all of them.
  Message history (context) is held client-side by `useChat` and re-sent each
  turn; there is no server-side store yet.
...
17. **`useChat` is one instance shared via React context.** `ChatProvider`
    (`app/lib/ai-chat.tsx`) runs `useChat` once; `Toolbar`/`StatusBar`/`ChatPanel`
    consume it. Never call `useChat()` per-component or you get separate chat
    states.
...
2. **Auto-retry on compile error:** currently single-step — if the LLM's
   OpenSCAD fails, the error is shown and the user must ask again. Consider
   `stopWhen: stepCountIs(3)` on `streamText` so the model can self-correct from
   its own stderr within one turn.
3. **Real export:** add `GET /api/export/:meshId?format=stl|obj|3mf` (serve the
   cached STL or re-render) and wire `useModelStore.exportModel` to it; the
   toolbar export toast currently says "dummy".
4. **Mesh delivery hardening:** large `$fn` meshes are JSON arrays over HTTP.
   Switch `/api/mesh/:id` to a binary `Float32Array`/ArrayBuffer response (or
   gzip) if turns get slow.
5. **Real WebSocket transport** replacing `DummyWebSocketClient` (for CAD
   progress / long renders), or keep HTTP streaming for everything.
6. Embed the Hono backend in Electron's main process (import `app` from
   `server/app.ts`, call `serve()` in-process) so the desktop app is
   self-contained.
7. Persist the resizable-panel split and (Phase 2) projects. Code editor +
   live sync (Phase 3). Build123D backend as an alternative `ModelingBackend`.
