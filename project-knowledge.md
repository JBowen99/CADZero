...
SDK over OpenRouter. The AI conversation is real (streaming + context); the
**CAD kernel / mesh generation is still mocked** — geometry is not yet produced
from chat. See `AI CAD MVP Project Specification.md` for the full vision.
...
> kernel can still land as a Python subprocess/service (or stay in Node) later.
...
│   ├── ChatPanel.tsx        # message list (ScrollArea) + composer; driven by useChatContext
│   ├── ChatMessage.tsx      # renders AI SDK UIMessage parts (text)
│   ├── CodeBlock.tsx        # kept for later CAD-code display (currently unused)
│   └── StatusBar.tsx
├── dummy/
│   └── ai.ts                # DummyBackend implements ModelingBackend interface (mesh/export only now)
├── lib/
│   ├── utils.ts             # cn() helper (required by shadcn)
│   └── ai-chat.tsx          # ChatProvider + useChatContext (single useChat instance)
├── services/
│   └── websocket.ts         # DummyWebSocketClient — swap for real WS later (CAD transport)
├── store/
...
├── routes/
│   ├── home.tsx             # the workspace (index route); wrapped in <ChatProvider>
...
├── app.ts                    # Hono app (runtime-agnostic): CORS + POST /api/chat + GET /api/health
├── index.ts                  # Node bootstrap only (serve() via @hono/node-server) — standalone entry
├── env.ts                    # reads OPENROUTER_API_KEY / _MODEL / PORT / ALLOWED_ORIGIN; assertConfig()
├── system-prompt.ts          # CAD-assistant system prompt
...
- **CAD/mesh is still mocked.** `useChatStore` was removed; chat no longer
  drives the viewport. The dummy `create`/`modify` keyword logic in
  `app/dummy/ai.ts` is dormant (only `export`/`setBackend`/`render`/`clear` are
  used by `useModelStore`). The viewport shows an empty state until the CAD
  kernel is wired (planned: AI calls a tool that returns a mesh, then
  `useModelStore.setMesh` is called from an `onFinish`/tool handler).
- **Dummy AI is keyword-based** (`app/dummy/ai.ts`): `cube|box`, `cylinder`,
  ...
  the vocabulary here when reviving mesh generation.
...
| `app/dummy/ai.ts` create/modify | **Mocked/dormant** — keyword mesh generator, no longer called |
| `app/services/websocket.ts` | **Mocked** — `DummyWebSocketClient`; becomes real WS for CAD transport |
| Export in `useModelStore`   | **Mocked** — returns a fake STL/OBJ/3MF blob             |
| Connection status           | **Mocked** — reflects the dummy WS, not the AI backend    |

The dummy `DummyBackend.create/modify` still returns the `BackendResult` shape
the CAD kernel should emit, so wiring mesh generation later is a drop-in.

---

## Next steps

1. **CAD kernel:** add an AI tool (e.g. `generate_cad`) to the chat backend so
   the model returns CAD code + mesh; call `useModelStore.setMesh` from a tool
   handler. Revive `ChatMessage`'s `CodeBlock` for the returned CAD code.
2. Real WebSocket transport replacing `DummyWebSocketClient` (for CAD progress /
   mesh streaming) — or keep HTTP streaming for everything.
3. Embed the Hono backend in Electron's main process (import `app` from
   `server/app.ts`, call `serve()` in-process) so the desktop app is
   self-contained.
4. Persist the resizable-panel split (use `onLayoutChanged` + localStorage, or
   `autoSaveId`).
5. Project save/load (Phase 2). Code editor + live sync (Phase 3).
